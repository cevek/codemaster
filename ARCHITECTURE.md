...
- **Pre-warm size guard (resource-respect before the warm, t-333163; pruning-aware t-399909).**
  The `ts` plugin's LS warm is the dominant memory risk (above), and a bare `search_symbol` fans
  navto across ALL programs — so on a huge monorepo the FIRST such call warms a gigabyte-scale
  program that (a) OOM-crashes the in-process daemon (an in-process OOM is uncatchable — §1) and (b)
  even in `process` mode squats that memory until idle-TTL eviction, for a throwaway discovery query
  `symbols_overview` answers with no warm. So a DEFAULT (navto) `search_symbol` CHEAPLY estimates the
  **post-pruning PEAK** — the files that will ACTUALLY build — BEFORE warming, and gates on THAT, not
  the total surface. The peak is pruning-aware because the t-167395 discovery prune collapses a
  loose-root monorepo's whole fan-out into the single primary program (which subsumes the in-root
  surface): `peakFiles = willPrune ? primary.fileNames().length : Σ program.fileNames().length`
  (each program owns its own checker, so an un-pruned fan-out's memory ≈ the SUM of its file-sets —
  the union surface undercounts the overlap and would miss that OOM). `willPrune` is predicted
  pre-warm by a SOUND-SUFFICIENT no-build check — the primary's `fileNames()` glob ⊇ the §10
  git-source surface (`fileNames ⊆ getSourceFiles`, so glob-coverage ⇒ the real prune engages) —
  reading `fileNames()` globs + one git listing, never a `getProgram()`/type-checker warm, never a
  `parseJsonConfigFileContent` tree-scan (§19-bounded — file-COUNT not bytes, since per-file sizing
  is the O(surface) hang-class §19 forbids; measured ~300 MB vs the ~1 GB+ warm it guards). Over
