...
- **L5 — MCP (`src/mcp/`)** — фасад с тремя MCP-инструментами.
...
## 7. Публичная поверхность: op · status · batch

Каждый schema MCP-инструмента + примеры загружаются в контекст агента **каждую сессию,
вечно** — фиксированный token-tax. Поэтому поверхность ровно **три инструмента**:

- **`op({ name, args, ...flags })`** — единый диспетчер. `name` — это enum активных op'ов
  в данном репо (зависит от того, какие плагины загружены); `args` — op-специфичные
  аргументы (zod-валидируются op'ом); `flags` — модификаторы вывода (`apply` для
  мутирующих, `verbosity`, `fields`, `format`, `debug`).
- **`status()`** — first-contact манифест. Перечисляет активные плагины, op-каталог для
  данного репо (имена + args schema + что делает), debug-неймспейсы. Преподаёт op-каталог
  **динамически**, поэтому ops стоят ноль standing context'а — только три инструмента.
- **`batch(requests)`** — список op-вызовов в одном round-trip'е. Результаты приходят в
  порядке запроса. Свежесть каждого затронутого плагина фиксируется один раз на входе
  batch'а, так что весь batch видит консистентный view (per-plugin).

Никаких top-level инструментов под отдельные ops (типа `find_usages` как самостоятельный
MCP-tool). Каждый op диспатчится через `op`; каталог обнаруживается через `status`.
