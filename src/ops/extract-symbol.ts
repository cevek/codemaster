...
export const extractSymbolOp = defineOp<ExtractArgs, JsonValue>({
  ...
  notes: [
    ...
    'capture-safe: each rewritten import (the source relinked to the new file, and consumers) is re-resolved over the post-edit tree — if one lands on a DIFFERENT same-named, type-compatible export the sites are listed under `captures` and apply is REFUSED. summaryOnly:true returns the verdict + ONE merged `touched` list (each file with its +added/-removed line counts; a moved-away/deleted source marked `(removed)`) instead of the full diff.',
