...
// over `React.ComponentProps<'button'>` inherits ~290 members it cannot act on); what that omits
// is COUNTED (`hiddenExternal`) and carries its own escape hatch — narrowing is never silent (§3.4).
...
export const findUnusedPropsOp = defineOp({
  ...
  summary:
    'Declared props of a React component that no JSX call-site passes (dead props) — the REPO-declared ones by default (`includeExternal` for the inherited DOM/aria surface, `prop:` to ask about named props); spread/opaque-ref demotes to partial',
  mutating: false,
  requires: ['react'],
  ...
  notes: [
    ...
    "DEFAULT VIEW = props the REPO declares. A prop whose DECLARATION file sits under `node_modules` or outside the repo root is omitted and counted as `hiddenExternal` (+ a note) — a wrapper over `React.ComponentProps<'button'>` otherwise buries its one own prop under ~290 DOM/aria members nobody can delete. `includeExternal:true` lists them; every row carries `external` so the two are told apart (`inherited` is a DIFFERENT claim — heritage — and is absent for an anonymous intersection).",
  ...
  async run(ctx, args) {
    ...
    try {
      ...
      if (capped !== undefined) {
        notes.push(`declared-member set capped at ${capped.shown}/${capped.total}`);
      }
      if (view.hiddenExternal !== undefined) {
        // The omission the default view performs — stated, never silent (§3.4). Under a capped
        // member set the count is a floor: unseen members could be external too.
        notes.push(
          `${capped !== undefined ? '≥' : ''}${view.hiddenExternal} unused prop(s) hidden — declared outside the repo's own source (node_modules / outside the root); pass includeExternal:true to list them`,
        );
      }
      if (view.undetermined !== undefined) {
        notes.push(
          `${view.undetermined.join(', ')}: not in the declared-member set — but that set was capped, so this is NOT a claim that the prop is undeclared`,
        );
      }
