...
export const cssCascadeOp = defineOp({
  ...
  notes: [
    ...
    'scopeable by pathInclude/pathExclude (globs over the .scss path); the owning sheet is always searched. Searches the currently-INDEXED .scss set (a just-created sheet may be missed until reindex; a `.css`/`.sass` sheet only when named directly as {file}). A no-match answer is `partial`, not a proven absence.',
  ],
