...
Honesty contract: results carry explicit freshness, confidence (certain/partial/dynamic/unresolved) and truncation; a FAIL means codemaster could not do it — fall back to your own tools then. A '!! CANNOT CLAIM' line means the answer is about ONE of several same-named declarations (the candidate page was cut) — every count/emptiness in it is a floor, whatever the payload's own 'complete' says; re-address with name+file or file:line:col. Query directly; do not delegate codemaster lookups to file-reading subagents.
