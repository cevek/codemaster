...
// to handle — `.prettierignore`d, or an extension with no inferred parser — returns
// `ok(null)` (a skip, not a failure): the touched file is still written, just unformatted.
// A broken `.prettierrc` / bad parser option becomes a `ToolFailure` (§3.6), never a throw
// that could kill a whole batch over one bad config (front-renamer's per-file isolation).
