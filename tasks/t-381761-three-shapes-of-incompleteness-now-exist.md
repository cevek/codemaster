...
FIELD CONFIRMATION from the third implementor: the blocker is the DISCRIMINATOR, not the prose. A shared coverage vocabulary that hard-codes its UNIT cannot be reused by a second fanning op even when both ARE fanning ops — the reference-denominated scan is a fan, and it still could not take the structure, because `scanCompleteness` decides emptiness on `walkedFiles === 0`. So the axis that matters is not fan-vs-single-program (the first refusal) nor one-program-vs-union (the second): it is the UNIT the completeness verdict is computed from. Any consolidation that keeps a unit-specific discriminator inside the shared type will hit this again on the fourth op. That is an argument for option (b) in this task — extract the INVARIANT (a completeness verdict derives from "the walk finished", never from "the count is zero") and leave each op its own denominator — over option (a).
